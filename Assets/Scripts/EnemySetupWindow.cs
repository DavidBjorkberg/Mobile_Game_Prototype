...
public class EnemySetupWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Delete latest Enemy", GUILayout.Width((position.width - position.width / 4))
            , GUILayout.Height(position.height / 5.3f)))
        {
            EnemyHandler enemyHandler = GameObject.Find("EnemyHandler").GetComponent<EnemyHandler>();
            enemyHandler.DestroyLastEnemy();
        }
        if (GUILayout.Button("Delete all enemies", GUILayout.Width(position.width / 4)
            , GUILayout.Height(position.height / 5.3f)))
        {
            EnemyHandler enemyHandler = GameObject.Find("EnemyHandler").GetComponent<EnemyHandler>();
        ...
        if (Selection.activeGameObject != null)
        {
            ...
            else if (Selection.activeGameObject.TryGetComponent(out TargetEnemy waypoint))
            {
                ...
                if (waypoint.owner.GetComponent<Enemy>().waypoints.Count > 0)
                {
                    ...
                    = waypoint.owner.GetComponent<Enemy>().waypoints[waypoint.owner.GetComponent<Enemy>().waypoints.Count - 1];
                }
                else
                {
